...
```lua
local mrpd_locker_rooms = exports.ds_blipcreator:getBlip(1)
local mrpd_locker_rooms = exports.ds_blipcreator:getBlipFromName('mrpd locker rooms')
```

- Set door state (0: unlocked, 1: locked)

```lua
TriggerEvent('ds_blipcreator:setBlip', mrpd_locker_rooms.id, state)
```

- Listen for event when door is toggled

```lua
AddEventHandler('ds_blipcreator:stateChanged', function(source, doorId, state, usedItem)
